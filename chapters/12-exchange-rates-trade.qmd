...
# PA.NUS.PPCD.D = PPP conversion factor, GDP (LCU per international $)
...
ppp_data <- WDI(
  country = g20_codes,
  indicator = c("PA.NUS.PPCD.D", "PA.NUS.FCRF"),
  start = 2022, end = 2022
)

ppp_data <- ppp_data |>
  rename(ppp_rate = PA.NUS.PPCD.D,
...
# The indicator code may vary; check available datasets
reer <- oecd_get(
  dataset = "MEI",
  filter = list(
    LOCATION = "GBR",
...
reer_multi <- oecd_get(
  dataset = "MEI",
  filter = list(
    LOCATION = c("GBR", "USA", "DEU", "JPN", "FRA"),
    SUBJECT = "CCRETT01",
    FREQUENCY = "M"
  )
) |>
