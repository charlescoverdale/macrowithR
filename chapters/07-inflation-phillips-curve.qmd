...
The CPI data arrives as an index level, so we need to compute the annual inflation rate — the percentage change over 12 months. We also extract a year-month date for merging.

```{r}
inflation <- cpi |>
  arrange(date) |>
  mutate(
    inflation = (value / lag(value, 12) - 1) * 100
  ) |>
...
phillips_data <- inflation |>
  inner_join(unemp, by = "date", suffix = c("_cpi", "_unemp")) |>
