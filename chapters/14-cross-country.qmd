...
# ONS publishes multi-factor productivity data
# which includes output, capital services, labour input, and MFP
...
gdp <- ons_gdp() |>
  filter(freq == "q") |>
  arrange(date) |>
  mutate(gdp_growth = (value / lag(value, 4) - 1) * 100)
...
panel_raw <- WDI(
  indicator = c(
    ...
    secondary_enrol = "SE.SEC.ENRR"        # Secondary school enrolment
  ),
  start = 2000, end = 2020,
  extra = TRUE
)
