...
debt_gdp <- pf |>
  filter(variable == "Public sector net debt (% of GDP)") |>
  filter(date >= as.Date("1990-01-01"))

ggplot(debt_gdp, aes(x = date, y = value)) +
  ...
  geom_vline(xintercept = as.Date("2020-03-23"),
             linetype = "dashed", colour = "grey40") +
  annotate("text", x = as.Date("2008-09-15"), y = 90,
           label = "Financial\ncrisis", hjust = -0.1,
           size = 3, colour = "grey40") +
  annotate("text", x = as.Date("2020-03-23"), y = 90,
           label = "COVID-19\npandemic", hjust = -0.1,
           size = 3, colour = "grey40") +
...
decomposition <- pf |>
  filter(variable %in% c(
    ...
    "Central government net interest payments (% of GDP)",
...
```{r}
# Download OBR forecasts from successive fiscal events
obr_forecasts <- obr_forecasts()

head(obr_forecasts)
```

```{r}
# Filter for GDP growth forecasts
gdp_forecasts <- obr_forecasts |>
  filter(variable == "Real GDP growth (%)") |>
  select(forecast_date, target_year, value)

# Get actual GDP growth outcomes
...
forecast_eval <- gdp_forecasts |>
  ...
  mutate(
    ...
    horizon = target_year - as.numeric(format(forecast_date, "%Y"))
  )
```

```{r}
# Plot forecast errors by horizon
ggplot(forecast_eval, aes(x = target_year, y = forecast_error)) +
  geom_col(fill = "#2196F3", alpha = 0.7) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  facet_wrap(~paste0(horizon, "-year ahead")) +
...
receipts_main <- receipts |>
  filter(tax %in% c("Income Tax", "VAT", "Corporation Tax")) |>
  filter(date >= as.Date("2010-01-01"))

ggplot(receipts_main, aes(x = date, y = value, colour = tax)) +
  geom_line(linewidth = 0.6) +
  labs(title = "UK Monthly Tax Receipts",
       subtitle = "£ millions, nominal",
...
total_receipts <- receipts |>
  ...
  arrange(date) |>
  mutate(yoy_growth = 100 * (total / lag(total, 12) - 1)) |>
...
ggplot(recent_years,
  ...
  labs(title = "Cumulative Tax Receipts by Fiscal Year",
       subtitle = "£ billions, April to March",
